...
"""
    ...
    def test_preview_switching_reuses_output_cards_instead_of_rebuilding_grid(self) -> None:
        ...
        self.assertIn("const taskChanged = String(state.previewTask?.task_id || \"\") !== String(task?.task_id || \"\")", source)
        self.assertIn("const preservePreviousImages = !taskChanged && previousOutputCount === outputUrls.length", source)
